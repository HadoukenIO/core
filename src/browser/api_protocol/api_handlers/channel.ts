...
export class ChannelApiHandler {
    private readonly actionMap: ActionSpecMap = {
        'create-channel': this.registerChannel,
    };
    ...
    private registerChannel(identity: Identity, message: any, ack: any, nack: any): void {
        const { payload } = message;
        const { channelName } = payload;

        const providerIdentity = Channel.registerChannel(identity, channelName);
