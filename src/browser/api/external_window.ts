...
*/
function findExistingNativeWindow(uuid: string): Shapes.NativeWindowInfoLite|undefined {
  ...
  const win = prelaunchedProcess
    ...
    : allNativeWindows.find(win => getNativeWindowInfoLite(win).uuid === uuid);
...
export function getExternalWindow(identity: Identity): Shapes.ExternalWindow {
  ...
  if (!externalWindow) {
    const nativeWinObj = findExistingNativeWindow(uuid);
